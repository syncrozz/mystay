...
import { getDayContextLabel, getDateForDay, getDayOptionsForStay } from '../utils/formatters';
  ...
  return (
    <div id="agenda-board" className="space-y-6">
      ...
      {backlogItems.length > 0 && (
        (() => {
          const targetDayNum = selectedDay === 0 ? 1 : selectedDay;
          const targetDateInfo = getDateForDay(stay.startDate, targetDayNum);
                  ...
                  return (
                    ...
                    >
                      ...
                      <button
                        ...
                        title={`Masukkan ke ${targetDateInfo.dateFormatted}`}
                      >
                        <Plus className="w-3 h-3" />
                        <span>{targetDateInfo.dayOfMonth} {targetDateInfo.monthShort}</span>
                      </button>
                    </div>
                  );
                })}
      ...
      <div className="bg-white p-3.5 sm:p-4 rounded-2xl border border-slate-200 shadow-2xs space-y-3">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="flex items-center gap-1.5 overflow-x-auto pb-1 sm:pb-0 scrollbar-none">
            ...
            {Array.from({ length: daysCount }).map((_, idx) => {
              ...
              return (
                <button
                  ...
                  className={`px-3 sm:px-3.5 py-2 rounded-xl text-xs font-bold transition-all shrink-0 flex items-center gap-1.5 border cursor-pointer ${
                    isSelected
                      ? isTravel
            ...
            >
              Semua Tarikh ({agendaItems.filter((i) => (i.dayNumber || 0) > 0).length})
            </button>
          </div>
        ...
        {activeDayContext && selectedDay > 0 && (
          (() => {
            const activeDateInfo = getDateForDay(stay.startDate, selectedDay);
            return (
              <div
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <span className="font-black text-slate-900">
                      📅 {activeDateInfo.dateFormatted}
                    </span>{' '}
                    <span className="font-semibold text-slate-600">
                      ({activeDateInfo.secondaryLabel}) · {activeDayContext.icon} {activeDayContext.label}
                    </span>
